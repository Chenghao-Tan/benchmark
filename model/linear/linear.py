    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._output_activation_name not in {"softmax", "sigmoid"}:
            raise ValueError(
                "LinearModel output_activation must be 'softmax' or 'sigmoid'"
            )
        if self._output_activation_name == "sigmoid" and self._criterion_name != "bce":
            raise ValueError(
                "LinearModel output_activation='sigmoid' requires criterion='bce'"
            )
        if self._output_activation_name == "softmax" and self._criterion_name == "bce":
            raise ValueError(
                "LinearModel output_activation='softmax' is incompatible with criterion='bce'"
            )
        ...
        with seed_context(self._seed):
            ...
            if self._output_activation_name == "sigmoid":
                if len(self.get_class_to_index()) != 2:
                    raise ValueError(
                        "LinearModel output_activation='sigmoid' supports binary classification only"
                    )
