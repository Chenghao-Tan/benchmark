...
@register("credit")
class CreditDataset(DatasetObject):
    """Load the bundled Credit dataset and its YAML metadata.
    ...
    def __init__(self, path: str = "./dataset/credit/", **kwargs):
    ...
    def _read_df(self, path: str) -> pd.DataFrame:
        return pd.read_csv(Path(path) / "credit.csv")
