...
@register("german_modified")
class GermanModifiedDataset(DatasetObject):
    def __init__(self, path: str = "./dataset/german_modified/", **kwargs):
    ...
    def _read_df(self, path: str) -> pd.DataFrame:
        return pd.read_csv(Path(path) / "german_modified.csv")
