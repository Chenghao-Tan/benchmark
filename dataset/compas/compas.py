...
@register("compas")
class CompasDataset(DatasetObject):
    """Load the bundled COMPAS dataset and its YAML metadata.
    ...
    def __init__(self, path: str = "./dataset/compas/", **kwargs):
    ...
    def _read_df(self, path: str) -> pd.DataFrame:
        return pd.read_csv(Path(path) / "compas.csv")
